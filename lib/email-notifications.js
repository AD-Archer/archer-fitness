...
export class EmailNotificationManager {
  ...
  async sendPasswordResetEmail(email, resetUrl) {
    const subject = '🔐 Reset your Archer Fitness password'
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #2563eb;">Password Reset Request</h2>
        <p>We received a request to reset the password associated with this email address.</p>
        <p>If you made this request, click the button below to choose a new password. This link will expire in 60 minutes.</p>
        ...
        <p style="color: #6b7280; font-size: 14px;">For your security, this link will expire 60 minutes after this email was sent.</p>
      </div>
    `
  ...
  async sendPasswordChangedEmail(email) {
    const subject = '✅ Your Archer Fitness password was updated'
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #16a34a;">Password Updated</h2>
        <p>Your Archer Fitness password was just changed successfully.</p>
        <p>If you made this change, no further action is required.</p>
        <p>If you didn't update your password, please reset it immediately and contact support at antonioarcher.dev@gmail.com.</p>
        <p style="color: #6b7280; font-size: 14px;">Note: This email is not monitored. For assistance, please reach out to antonioarcher.dev@gmail.com.</p>
      </div>
    `
  ...
  async sendEmailVerificationEmail(email, verifyUrl, code) {
    const subject = '🔐 Verify your Archer Fitness email'
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #2563eb;">Verify Your Email Address</h2>
        <p>Thanks for updating your email address. To complete the process, please verify this email address.</p>
        
        <div style="background: #f3f4f6; border-radius: 8px; padding: 24px; margin: 24px 0;">
          <p style="margin: 0 0 16px 0; font-weight: 600;">Option 1: Click the button below</p>
          <p style="text-align: center; margin: 0 0 24px 0;">
            <a href="${verifyUrl}" style="background: #2563eb; color: white; padding: 12px 32px; text-decoration: none; border-radius: 6px; display: inline-block;">Verify Email Address</a>
          </p>
          
          <p style="margin: 24px 0 8px 0; font-weight: 600;">Option 2: Enter this code in the app</p>
          <div style="background: white; border: 2px solid #2563eb; border-radius: 6px; padding: 16px; text-align: center;">
            <span style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #2563eb;">${code}</span>
          </div>
          <p style="margin: 8px 0 0 0; font-size: 12px; color: #6b7280; text-align: center;">This code expires in 60 minutes</p>
        </div>
  ...
  async sendEmailVerifiedEmail(email) {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>You can now use <strong>${email}</strong> to sign in to your Archer Fitness account.</p>
        <p><a href="${process.env.NEXTAUTH_URL}/auth/signin" style="background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">Go to Sign In</a></p>
        <p style="color: #6b7280; font-size: 14px;">Thanks for being part of Archer Fitness! 💪</p>
      </div>
    `
