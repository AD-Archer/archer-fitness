...
import type { ExercisePreview, ProgressionNode, WorkoutSessionSummary, NodeStatus } from "@/lib/progression/types"
...
export function ProgressionExperience() {
  ...
  const [selectedBranchId, setSelectedBranchId] = useState(progressionBranches[0]?.id ?? "")
  const [selectedNodeId, setSelectedNodeId] = useState(progressionBranches[0]?.milestones[0]?.id ?? "")
  ...
  const [exerciseExamples, setExerciseExamples] = useState<Record<string, ExercisePreview | null>>({})
  const [examplesLoading, setExamplesLoading] = useState(false)
  const [storedProgress, setStoredProgress] = useState<Record<string, { status: NodeStatus; completionCount: number }>>({})
  const [profile, setProfile] = useState<{ alias: string; crowns: number; totalXp: number } | null>(null)
  ...
  const exampleCacheRef = useRef<Record<string, ExercisePreview | null | undefined>>({})
  ...
  const loadProgressionData = useCallback(async () => {
    try {
      ...
      if (sessionsRes.status === 401 || templatesRes.status === 401 || progressRes.status === 401) {
      ...
      if (!sessionsRes.ok) {
        throw new Error(`Failed to load workout sessions (${sessionsRes.status})`)
      }
      if (!templatesRes.ok) {
        throw new Error(`Failed to load workout templates (${templatesRes.status})`)
      }
      if (!progressRes.ok && progressRes.status !== 404) {
        throw new Error(`Failed to load progression state (${progressRes.status})`)
      }
      ...
      setTemplateNames(templatePool.map((tpl: { name?: string }) => (tpl.name ?? "").toLowerCase()))

      if (progressJson?.profile) {
        setProfile(progressJson.profile)
      }
      if (Array.isArray(progressJson?.records)) {
        const overrides: Record<string, { status: NodeStatus; completionCount: number }> = {}
        progressJson.records.forEach((record: { nodeId?: string; status?: string; completionCount?: number }) => {
    ...
    const loadLeaderboard = async () => {
      try {
        ...
        const nodes = (data.nodes || data.leaderboard || []).reduce(
          (acc: Record<string, { percent: number; clearedCount: number }>, entry: { nodeId: string; percent: number; clearedCount: number }) => {
  ...
  const progressionState = useProgressionState(progressionBranches, sessions, storedProgress)

  const templateNameSet = useMemo(() => new Set(templateNames), [templateNames])

  const selectedBranch = progressionBranches.find((branch) => branch.id === selectedBranchId) ?? progressionBranches[0]
  const selectedNode = selectedBranch?.milestones.find((node) => node.id === selectedNodeId) ?? selectedBranch?.milestones[0]
  const selectedNodeState = selectedNode ? progressionState.nodeStates[selectedNode.id] : undefined

  const prerequisiteLabels = selectedNode?.prerequisites.map((id) => nodeDictionary.get(id)?.name ?? "Milestone") ?? []

  const syncedTemplateCount = progressionBranches
    .flatMap((branch) => branch.milestones)
    .filter((node) => templateNameSet.has(node.template.name.toLowerCase()))
    .length
  ...
  useEffect(() => {
    const payload = Object.entries(progressionState.nodeStates)
      ...
      .filter(Boolean) as Array<{ nodeId: string; status: NodeStatus; completionCount: number; xpEarned: number }>
    ...
    fetch("/api/progression/progress", {
      ...
      body,
    }).catch((error) => logger.warn("Failed to sync progression progress", error))
  ...
  const fetchExerciseExample = useCallback(async (name: string) => {
    ...
    try {
      const res = await fetch(`/api/workout-tracker/exercises?limit=5&search=${encodeURIComponent(name)}`)
      if (!res.ok) throw new Error("Failed to fetch exercise example")
      const data = await res.json()
      const pool: ApiExercise[] = [...(data.userExercises || []), ...(data.predefinedExercises || [])]
      const keyword = normalized.split(" ")[0]
      const exact = pool.find((exercise) => exercise.name?.toLowerCase() === normalized)
      const fallback = pool.find((exercise) => exercise.name?.toLowerCase().includes(keyword))
  ...
  useEffect(() => {
    ...
    const names = Array.from(new Set(selectedNode.template.exercises.map((exercise) => exercise.name.toLowerCase())))
    const unresolved = names.filter((name) => exampleCacheRef.current[name] === undefined)
  ...
  const handleSaveTemplate = async (node: ProgressionNode) => {
    try {
      ...
      toast.success("Template saved. Find it under Workout → Custom templates.")
      setTemplateNames((prev) => Array.from(new Set([...prev, node.template.name.toLowerCase()])))
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
      <ProgressionSummary
        ...
        syncedTemplates={syncedTemplateCount}
        workoutSessionsTracked={sessions.length}
        playerAlias={profile?.alias ?? leaderboardStats.currentPlayer?.alias ?? undefined}
      ...
      <Card className="p-6 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-muted-foreground">Currently exploring</p>
            ...
            {selectedNode && selectedNodeState && (
              <div className="rounded-2xl border px-4 py-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {selectedNodeState.completionCount}/{selectedNode.targetSessions} runs
                </p>
              </div>
            )}
            <Button variant="outline" onClick={() => setDetailOpen(true)} disabled={!selectedNode}>
              View node details
            </Button>
          </div>
        </div>
      ...
      <NodeDetailDialog
        ...
        templateExists={selectedNode ? templateNameSet.has(selectedNode.template.name.toLowerCase()) : false}
      ...
      <Card className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border bg-card/70 px-5 py-4 text-sm text-muted-foreground">
        <div>
          ...
          <p>{sessions.length} workouts scanned • new runs update the tree instantly.</p>
        </div>
