    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-semibold">
              {state.completionCount}/{node.targetSessions} runs (
              {progressPercent}%)
            </span>
          </div>
                  ...
                  >
                    <div>
                      ...
                      {exercise.instructions && (
                        <p className="text-xs mt-1">{exercise.instructions}</p>
                      )}
                    </div>
                    <ExamplePreview
                      example={example}
        ...
        <div className="flex flex-col gap-3 sm:flex-row flex-shrink-0">
          ...
          >
            {startingSessionFor === node.id
              ? "Starting session..."
              : "Start practice in Tracker"}
          </Button>
          <Button
            variant="outline"
            className="flex-1"
...
function ExamplePreview({
  ...
  example?: ExercisePreview | null;
  loading: boolean;
}) {
  if (loading && !example) {
    return (
      <p className="text-xs text-muted-foreground">
        Searching the exercise database…
      </p>
    );
  }
  ...
  if (!example) {
    return (
      <p className="text-xs text-muted-foreground">
        No preview yet. Add this move once to save it.
      </p>
    );
  }
