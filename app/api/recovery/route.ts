...
async function buildRecoveryResponse(
  ...
  for (const session of sessions) {
    ...
    for (const sessionExercise of session.exercises) {
      // Skip if exercise is null (was deleted)
      ...
      const setCount =
        sessionExercise.sets?.length || sessionExercise.targetSets || 1;
      ...
      for (const rawName of bodyPartNames) {
        ...
        if (!bodyPartAggregates.has(key)) {
          bodyPartAggregates.set(key, {
            ...
            sevenDaySessions:
              performedAt >= sevenDaysAgo ? new Set([session.id]) : new Set(),
        ...
        aggregate.lastWorkout = aggregate.lastWorkout
          ? new Date(
              Math.max(aggregate.lastWorkout.getTime(), performedAt.getTime()),
            )
          : performedAt;
  ...
  const bodyParts: BodyPartInsight[] = Array.from(
    bodyPartAggregates.values(),
  ).map((aggregate) => {
    ...
    return {
      ...
      lastWorkout: aggregate.lastWorkout
        ? aggregate.lastWorkout.toISOString()
        : null,
      hoursSinceLast,
      ...
      averageSets:
        aggregate.sessionIds.size > 0
          ? Number((aggregate.totalSets / aggregate.sessionIds.size).toFixed(1))
          : 0,
      feedback: feedback
        ? {
  ...
  const summary: RecoverySummary = bodyParts.reduce(
    (acc, part) => {
      acc[`${part.status}Count` as const] =
        acc[`${part.status}Count` as const] + 1;
  ...
  summary.nextEligibleInHours.sort(
