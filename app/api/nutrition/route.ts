...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalCalories = nutritionLogs.reduce(
      (sum: any, log: any) => sum + log.calories,
      0,
    );
    const totalProtein = nutritionLogs.reduce(
      (sum: any, log: any) => sum + log.protein,
      0,
    );
    const totalCarbs = nutritionLogs.reduce(
      (sum: any, log: any) => sum + log.carbs,
      0,
    );
    const totalFat = nutritionLogs.reduce(
      (sum: any, log: any) => sum + log.fat,
      0,
    );

    const averageCalories =
      nutritionLogs.length > 0 ? totalCalories / nutritionLogs.length : 0;
    const averageProtein =
      nutritionLogs.length > 0 ? totalProtein / nutritionLogs.length : 0;
    const averageCarbs =
      nutritionLogs.length > 0 ? totalCarbs / nutritionLogs.length : 0;
    const averageFat =
      nutritionLogs.length > 0 ? totalFat / nutritionLogs.length : 0;
