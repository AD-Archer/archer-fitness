...
const calculateDiffInDays = (target: Date, baseline: Date) => {
  return Math.floor(
    (startOfDay(target).getTime() - startOfDay(baseline).getTime()) / MS_IN_DAY,
  );
};
...
const expandRecurringItemForWeek = (
  ...
  const occurrences: Array<
    Omit<PrismaRecurringItem, "schedule"> & {
      id: string;
      originId: string;
      day: number;
      isVirtual: boolean;
  ...
  const pattern = (item.repeatPattern || "weekly") as
    | "daily"
    | "weekly"
    | "yearly";
  const interval =
    item.repeatInterval && item.repeatInterval > 0 ? item.repeatInterval : 1;
  const originWeekStart = startOfDay(item.schedule.weekStart);
  const originOccurrenceDate = startOfDay(
    new Date(originWeekStart.getTime() + item.day * MS_IN_DAY),
  );
  ...
  if (pattern === "weekly") {
    ...
    const daysOfWeek =
      item.repeatDaysOfWeek && item.repeatDaysOfWeek.length > 0
    ...
    for (let index = 0; index < 7; index++) {
      ...
      const totalDiffDays = calculateDiffInDays(
        occurrenceDate,
    ...
    for (let index = 0; index < 7; index++) {
      ...
      const yearsDiff =
        occurrenceDate.getFullYear() - originOccurrenceDate.getFullYear();
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!schedule) {
      logger.info("No schedule found, creating empty schedule:", {
        userId: session.user.id,
        weekStart: weekStartDate,
      });
    ...
    const recurringItems = (
      recurringCandidates as PrismaRecurringItem[]
    ).filter((item) => item.isRecurring);

    const virtualItems = recurringItems.flatMap((recurringItem) => {
      const occurrences = expandRecurringItemForWeek(
        recurringItem,
        weekStartDate,
        weekKey,
      );
      return occurrences
        .filter((occurrence) => {
          ...
          const signature =
            `${occurrence.title}-${occurrence.startTime}-${occurrence.day}`.toLowerCase();
    ...
    const combinedItems = [...normalizedBaseItems, ...virtualItems].sort(
      (a, b) => {
    ...
    const scheduleTimezone =
      ((schedule as any).timezone as string | undefined) || "UTC";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!schedule) {
      ...
      logger.info("Updating schedule timezone:", {
        scheduleId: schedule.id,
        oldTimezone: (schedule as any).timezone,
    ...
    if (items && Array.isArray(items)) {
      ...
      logger.info("Deleted existing items:", {
        scheduleId: schedule.id,
        ...
        await prisma.scheduleItem.createMany({
          data: items.map(
            (item: {
              ...
              repeatPattern: item.repeatPattern ?? null,
              repeatInterval: item.repeatInterval ?? null,
              repeatEndsOn: item.repeatEndsOn
                ? new Date(item.repeatEndsOn)
                : null,
