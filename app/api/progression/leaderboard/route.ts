...
export async function GET() {
  try {
    ...
    const [totalPlayers, nodeGroups, profiles, currentProfile] = await Promise.all([
    ...
    if (currentProfile) {
      const higher = await prisma.progressionProfile.count({ where: { totalXp: { gt: currentProfile.totalXp } } })
    ...
    return NextResponse.json({
      ...
      currentPlayer: currentProfile
        ? { alias: currentProfile.alias, rank: currentRank ?? 0, xp: currentProfile.totalXp }
        : null,
