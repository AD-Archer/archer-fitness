...
async function ensureSetOwnership(userId: string, params: { id: string; exerciseId: string; setId: string }) {
...
export async function PUT(
  ...
  try {
    ...
    const existingSet = await ensureSetOwnership(session.user.id, params)
    if (!existingSet) {
    ...
    const updatedExercise = await getUpdatedExercise(params.exerciseId)

    return NextResponse.json({ set: updatedSet, exercise: updatedExercise }, { status: 200 })
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    await prismaAny.$transaction(async (tx: any) => {
      await tx.exerciseSet.delete({ where: { id: params.setId } })
      ...
      await Promise.all(
        remainingSets.map((set: { id: string; setNumber: number }, index: number) =>
          set.setNumber === index + 1
            ? null
            : tx.exerciseSet.update({ where: { id: set.id }, data: { setNumber: index + 1 } })
        )
      )
    })
    ...
    return NextResponse.json({ success: true, exercise: updatedExercise }, { status: 200 })
  } catch (error) {
