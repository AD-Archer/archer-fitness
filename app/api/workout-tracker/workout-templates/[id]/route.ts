        ...
        if (!exerciseId) {
          ...
          if (!nameFromBody) {
            return NextResponse.json(
              {
                error: `Exercise at position ${
                  i + 1
                } missing exerciseId or name`,
              },
              { status: 400 }
            );
          }
          // Try user's custom exercise
          let exRec = await prisma.exercise.findFirst({
            where: { userId: session.user.id, name: nameFromBody },
            select: { id: true },
          });
          if (!exRec) {
            // Try predefined
            exRec = await prisma.exercise.findFirst({
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.workoutTemplate.delete({ where: { id: id } });
