...
export function DistributionAnalytics({
    ...
    const fetchMuscleGroupData = async () => {
      try {
        const response = await fetch(
          `/api/workout-tracker/workout-sessions?limit=50&timeRange=${timeRange}`
        );
        if (response.ok) {
          ...
          sessions.forEach((session) => {
            ...
            session.exercises.forEach((sessionEx) => {
              ...
              if (completedSets > 0) {
                ...
                logger.info(
                  `Exercise: ${sessionEx.exercise.name}, Completed Sets: ${completedSets}, Muscles:`,
                  sessionEx.exercise.muscles
                );
                ...
                logger.info(
                  `Primary muscles: ${primaryMuscles}, Secondary muscles: ${secondaryMuscles}`
                );
                ...
                primaryMuscles.forEach((muscle) => {
                  muscleGroupSets.set(
                    muscle,
                    (muscleGroupSets.get(muscle) || 0) + completedSets
                  );
                });
                ...
                secondaryMuscles.forEach((muscle) => {
                  muscleGroupSets.set(
                    muscle,
                    (muscleGroupSets.get(muscle) || 0) + completedSets * 0.5
                  );
                });
              }
            });
          ...
          muscleGroupSets.forEach((sets, muscleName) => {
            ...
            muscleGroupMap.set(
              normalizedName,
              (muscleGroupMap.get(normalizedName) || 0) + sets
            );
          });
          ...
          if (totalCompletedSets > 0) {
            const chartData: MuscleGroupData[] = Array.from(
  ...
  const normalizeMuscleGroup = (muscleName: string): string => {
    ...
    if (
      ...
      return "Back";
    if (normalized.includes("shoulder") || normalized.includes("deltoid"))
      return "Shoulders";
    if (
  ...
  const generateRecommendations = () => {
    ...
    if (highest.value > 40) {
      recommendations.push({
        type: "rest",
        message: `You need to rest your ${highest.name} (${
          highest.value
        }% of total volume). Consider taking 2-3 days off from ${highest.name.toLowerCase()} exercises to allow proper recovery.`,
      });
    ...
    if (lowest.value < 5 && lowest.name !== "Core") {
      recommendations.push({
        type: "focus",
        message: `You should focus on ${lowest.name} (only ${
          lowest.value
        }% of volume). Try adding 2-3 ${lowest.name.toLowerCase()} exercises to your next workout.`,
      });
    ...
    const chestVolume =
      muscleGroupData.find((g) => g.name === "Chest")?.value || 0;
    const backVolume =
      muscleGroupData.find((g) => g.name === "Back")?.value || 0;
    ...
    if (chestVolume > 0 && backVolume > 0) {
      ...
      if (chestBackRatio > 1.5) {
        recommendations.push({
          type: "balance",
          message: `You should focus on Back exercises. Your chest-to-back ratio is ${chestBackRatio.toFixed(
            1
          )}:1, which could lead to posture issues.`,
        });
      } else if (chestBackRatio < 0.67) {
        recommendations.push({
          type: "balance",
          message: `You should focus on Chest exercises. Your back-to-chest ratio is ${(
            1 / chestBackRatio
          ).toFixed(1)}:1, consider adding more pressing movements.`,
        });
      }
    }
    ...
    if (maxDifference < 20 && muscleGroupData.length >= 4) {
      recommendations.push({
        type: "success",
        message:
          "Great balance! Your training is well-distributed across muscle groups. Keep up the excellent work!",
      });
    }
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Loading your training distribution...
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Distribution of training volume by muscle group ({totalSets} total
              sets)
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="h-64 flex flex-col items-center justify-center text-muted-foreground">
                <div className="text-center">
                  {totalSets === 0 ? (
                    <>
                      <div>
                        No completed sets found in your workout sessions.
                      </div>
                      <div className="text-xs mt-2">
                        Complete some sets in your workouts to see muscle group
                        distribution!
                      </div>
                    </>
                  ) : (
      ...
      {recommendations.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              AI recommendations for optimal muscle group distribution
            </CardDescription>
          </CardHeader>
