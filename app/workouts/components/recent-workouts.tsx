    ...
    const fetchData = async () => {
      try {
        ...
        const sessionsResponse = await fetch(
          "/api/workout-tracker/workout-sessions?limit=10&visibility=all",
        );
        ...
        const templatesResponse = await fetch(
          "/api/workout-tracker/workout-templates?limit=10",
        );
        let templatesData: ApiTemplatesResponse = {
          userTemplates: [],
          predefinedTemplates: [],
        };
        ...
        const transformedSessions = sessionsData.reduce<WorkoutSession[]>(
          (acc, session) => {
            ...
            const isDiscarded =
              typeof session.isDiscarded === "boolean"
                ? session.isDiscarded
                : isSessionDiscarded();
            ...
            if (
              ...
              logger.info(
                `Filtering out ${session.name}: cancelled=${session.status === "cancelled"}, discarded=${isDiscarded}, archived=${session.isArchived}`,
              );
              return acc;
            }
            ...
            if (completionRateRaw <= 0 && session.status !== "completed") {
              logger.info(
                `Filtering out ${session.name}: completionRate=${completionRateRaw}, status=${session.status}`,
              );
              return acc;
            }
        ...
        const allTemplates = [
          ...
          ...templatesData.predefinedTemplates,
        ];
        const transformedTemplates = allTemplates.map(
          (template: ApiWorkoutTemplate) => ({
            id: template.id,
            name: template.name,
            description: template.description || template.name,
            difficulty: template.difficulty as
              | "beginner"
              | "intermediate"
              | "advanced",
  ...
  const handleRepeatWorkout = async (workoutId: string) => {
    try {
      const response = await fetch(
        `/api/workout-tracker/workout-sessions/${workoutId}/repeat`,
        {
          method: "POST",
        },
      );
      ...
      <CardContent>
        <div className="space-y-4">
          {recentWorkouts.map((workout, idx) => {
            ...
            const hasNewerStarted = recentWorkouts.some(
              (w, i) =>
                i < idx &&
                new Date(w.date).getTime() > new Date(workout.date).getTime(),
            );
            const completionRate = Math.round(
              computeCompletionRate(workout.exercises),
            );
            const isCompleted =
              workout.performanceStatus === "completed" ||
              workout.performanceStatus === "perfect" ||
              completionRate > 0;
            return (
              <div
                key={workout.id}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-2 sm:p-3 rounded-lg border bg-card/50 gap-2 sm:gap-3"
              >
                ...
                <div className="flex items-center gap-1 flex-wrap justify-end">
                  ...
                  <WorkoutDetailsModal
                    ...
                    onRepeat={
                      onRepeatWorkout
                        ? (session) => {
                            const matched = recentWorkouts.find(
