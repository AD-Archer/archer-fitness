  ...
  useEffect(() => {
    const fetchActive = async () => {
      try {
        ...
        const res = await fetch(`/api/workout-tracker/workout-sessions?date=${today}`)
        if(!res.ok) return
        const data: ApiSession[] = await res.json()
        const session = data.find(s => ['active','paused'].includes(s.status)) || null
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-2.5 flex items-start gap-2">
        <div className="mt-0.5 text-muted-foreground">
          {activeSession ? <RotateCcw className="h-4 w-4" /> : <Dumbbell className="h-4 w-4" />}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            ...
            <Button asChild size="sm" variant="ghost" className="h-6 px-2 text-[10px]">
              <Link href={activeSession ? `/track?sessionId=${activeSession.id}` : '/track'}>
                {activeSession ? 'Resume' : 'Start'} <ChevronRight className="h-3 w-3 ml-0.5" />
              </Link>
            </Button>
          </div>
          {loading ? (
            ...
            <p className="text-[10px] text-blue-600 dark:text-blue-400 truncate">{activeSession.name}</p>
          ) : (
